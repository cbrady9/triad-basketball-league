...
function renderResultsWidget(data) {
    ...
    if (recentGames.length > 0) {
        ...
        recentGames.forEach(game => {
            ...
            recentHtml += `<a href="${gameLink}" class="block p-3 bg-gray-700/50 rounded-md hover:bg-gray-700"><div class="flex justify-between items-center"><span class="text-gray-300 text-sm">${game['Team 1']} vs ${game['Team 2']}</span><div class="flex items-center space-x-4"><span class="text-gray-400 text-xs">${game['Date']}</span><span class="font-bold text-gray-200 text-sm">${game['Team 1 Score']} - ${game['Team 2 Score']}</span></div></div></a>`;
        });
        recentHtml += '</div>';
    } else {
...
function renderLeadersWidget(data) {
    ...
    if (!container || !data || data.length === 0) {
        container.innerHTML = '<h3 class="text-2xl font-semibold mb-4 text-gray-200">League Leaders</h3><div class="text-center py-8"><img src="https://images.undraw.co/undraw_analytics_re_dkf8.svg" alt="No stats available" class="mx-auto w-32 h-32 opacity-40"><p class="text-gray-400 mt-4">No stats available yet.</p></div>';
        return;
    }
    ...
    const leaderCategories = [
        { title: 'Points Per Game', leader: ppgLeader, statKey: 'PPG' },
        { title: 'Rebounds Per Game', leader: rpgLeader, statKey: 'RPG' },
        { title: 'Assists Per Game', leader: apgLeader, statKey: 'APG' },
    ];
        ...
        if (cat.leader) {
            ...
            html += `
                <div class="flex justify-between items-center text-sm p-3 bg-gray-700/50 rounded-md">
                    ...
                    <a href="${playerLink}" class="flex items-center space-x-2 group">
                        <span class="font-semibold text-gray-200 group-hover:text-sky-400">${playerName}</span>
                        <span class="font-bold text-sky-400 text-base">${statValue}</span>
                    </a>
                </div>
            `;
        }
...
async function initializeHomePage() {
    ...
    const playerStatsGID = getGID('PLAYER_STATS_GID', currentSeason); // GID for player stats
    ...
    try {
        const [
            ...
            fetchGoogleSheetData(SHEET_ID, scheduleGID, 'SELECT A, C, D, E, F, G, H'),
        ...
        if (scheduleData) { renderResultsWidget(scheduleData); } // Renamed from renderScheduleWidgets
