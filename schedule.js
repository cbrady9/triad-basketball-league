...
const SCHEDULE_GID = '1956260076'; // GID for the 'Schedule' tab

// The Google Visualization API query.
// 'SELECT A, B, C, D' means select columns A, B, C, and D.
// Adjust these column letters based on where your data is in the 'Schedule' sheet.
// For example, if 'Team Name' is in column A, 'Wins' in B, 'Losses' in C, 'Point Differential' in D.
const SCHEDULE_QUERY = 'SELECT A, B, E, F, G, H, I, J'; // Adjust as needed!
...
function renderScheduleTable(tableData) {
...
async function initializeSchedulePage() {
    const tableData = await fetchGoogleSheetData(SHEET_ID, SCHEDULE_GID, SCHEDULE_QUERY);
    if (tableData) {
