...
function renderStandingsTable(data) {
    ...
    const headerMap = { 'Games Played': 'GP', 'Logo URL': '' }; // Hide Logo URL header
    const orderedHeaders = ['Rank', 'Logo URL', 'Team Name', 'Games Played', 'Wins', 'Losses', 'Win %', 'Point Differential'];
    ...
    filteredData.forEach((row, index) => {
        ...
        orderedHeaders.forEach(header => {
            ...
            if (header === 'Rank') {
                ...
                const teamName = row[header] !== undefined ? row[header] : '';
                const logoUrl = row['Logo URL'] || 'https://i.imgur.com/p3nQp25.png';
                displayValue = `<a href="team-detail.html?teamName=${encodeURIComponent(teamName)}" class="flex items-center group"><img src="${logoUrl}" class="w-8 h-8 mr-3 object-contain"><span class="text-sky-400 group-hover:underline font-semibold">${teamName}</span></a>`;
            } else {
